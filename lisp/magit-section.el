...
(defvar magit-ellipsis-str (string magit-ellipsis))
...
(defun magit-section-show (section)
  ...
  (when-let ((beg (oref section content)))
    (remove-overlays (oref section start)
                     (oref section end)
                     'before-string magit-ellipsis-str)
    ...
    (when-let ((beg (oref section content)))
      (let ((end (oref section end)))
        (remove-overlays (oref section start)
                         (oref section start)
                         'before-string magit-ellipsis-str)
        (let ((o (make-overlay (1- beg) beg)))
          (overlay-put o 'evaporate t)
          (overlay-put o 'before-string magit-ellipsis-str)
          ;; (overlay-put o 'priority 0)
          ;; (overlay-put o 'priority '(0 . 0))
          )
...
(defun magit-section-make-overlay (start end face)
  ...
  (when (oref (get-text-property start 'magit-section) hidden)
    (when-let ((ov (--first (overlay-get it 'before-string)
                            (overlays-at (1- start)))))
